...
with sync_playwright() as p:
    ...
    pg=ctx.new_page()
    pg.goto(f"file:///{BASE}/baseline-version-j-dusk-mist.html".replace("\\","/"),wait_until="networkidle",timeout=15000)
    time.sleep(0.5)
    for pid,pname in pages:
        if pid!="onboarding":pg.evaluate(f"navigate('{pid}')");time.sleep(0.4)
        pg.screenshot(path=os.path.join(BASE,f"baseline-J-{pname}.png"),full_page=False)
        print(f"✓ {pname}")
    pg.close();b.close()
print("Done")
